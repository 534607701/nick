...
echo -e "${BLUE}========================================${NC}"
echo -e "${GREEN}   功耗采集服务安装脚本 v3.0 (张武定制版)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo -e "  服务器: ${GREEN}${SERVER_NAME}${NC}"
...
def collect_data():
    ...
    return {
        ...
        'data': {
            ...
            'cpu_power': round(cpu_other_power, 2),
            'gpu_count': len(gpus),
            'gpus': gpus,
...
echo -e "  修改配置: ${YELLOW}systemctl edit power-monitor.service${NC}"
echo -e "${BLUE}========================================${NC}"
